...
class PickSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for doi in all_dois:
            yield scrapy.Request(api_stub+doi+'/agency',callback=self.parse_agency,meta={'doi':doi,'api_stub':api_stub,'source_url':response.url})
    ...
    def parse_agency(self,response):
        if not(response.body_as_unicode()==u'Resource not found.'):
            ...
            if (ag==u'crossref'):
                yield scrapy.Request(api_stub+doi,callback=self.parse_crossref, meta={'doi':doi,'source_url':response.meta['source_url']})
            else:
                item = CherryItems.DoiItem()
                item['doi']=doi
    ...
    def parse_crossref(self,response):
        if not(response.body_as_unicode()==u'Resource not found.'):
            ...
            item = Items.CherryItems.DoiItem()
